...
- [x] 通知器领域
  ...
  - [x] 核心功能: 通知器
    ...
    - [x]  通知器功能，	
       	...
       	​			  （1）核心功能 ： 通知器，使用策略模式进行api隔离（不涉及分布式缓存

      ​              （2）装饰器：qps限制

    - [x] ​	 池化通知器

      ​    提供缓存和初始化，（注册是永久的，除非外部表示销毁，但通知器实例的池化是有时效的，销毁后可以通过缓存信息

    ​        重新初始化实例）     
    - [x] ​	服务定义，收束上述两个功能，提供领域统一接口
  ...
  - [ ] 设计缓存索引
    ...
    ​      数据特点，<k,V>  索引: 信息
       ...
       问题： 分布式缓存存什么？存实例？还是验证信息？

    ​       注册之后，执行节点不一定和注册实例的在同一个节点

    ​       但是如果是共享实例的话，开销会不会太大？毕竟相当于每一个节点都要备份一个，

    ​       即便忽略网络开销和延迟，但是实例有可能和节点强相关，如何解决？

       解决：

    ​     构思： 做成类似路由表的列表，信息中存储实例注册的节点，有点类似消费者和注册者，我们直接向目标节点进行通信消费。（这样处理之后，我无法看到水平拓展的优势，即路由-》节点-》消费，而且mq消费还是统一的，这意味着要么实现细颗粒度的队列进行消费+多节点注册，要么保证任意节点都能够使用注册的信息，即只校缓存限信息，并且注意保证redis信息不泄露）
    ...
    ​	
       ...
       通知器注册（注册信息包含通知器权限信息）-》 多节点消费（支持水平拓展）

    问题： 外部如何调用通知器节点上的通知器实例？每个节点上的通知器实例要立即销毁吗？
    ...
    ​      缓存击穿： key失效导致的大量并发lua脚本请求执行？
    
    ​       对于qps键由于使用lua进行原子执行，无需考虑，问题是之后的请求仍然可能读取缓存，进而产生缓存击穿、缓存雪崩等问题
    
    
  
- [x] 接入第三方
  
    - [x]  tgBot:  tgbot需要提前缓存chatID才能够主动通信。 传入参数进行解耦，chatId是每次通信都要传输的动态参数，验证信息则是固定参数
  - [x] 微信： 暂时放弃。 原因： 微信没有官方的个人号sdk，只有微信公众号sdk，暂时不去实现。
...
2，各个通知器资源的详细优化，例如何时关闭，关闭什么？


####   第二阶段  对接日报程序（11.29-12.3）



 - [ ]  api对接，功能测试
 - [ ] 性能测试，尝试为通知器项目，添加指标进行grafana进行可视化，
